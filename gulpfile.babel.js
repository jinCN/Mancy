...
const nodeResources = (() => {
  ...
  return _.chain(result.toString().trim().split(/\r?\n/))
    ...
    .filter((dep) => (dep.match(/node_modules/g) || []).length == 1 &&
      !dep.match(/typescript/))
    .map((dep) => `${dep}/**/*.{js,cljs,css,json,svg,png,gif,woff2,otf,ttf,woff,eot}`)
    .value()
})()
...
let zipExe = async function (dists) {
  return Promise.all(_.map(dists, (dist) => {
    ...
    return spawn('zip', [
      `${PATHS.DIST}/${name}.zip`, '-r', `${PATHS.DIST}/${name}`])
  }))
}
...
let releaseCommit = async function () {
  ...
  await spawn('git', [
    'commit', '-m', `Release v${Config.version}`, 'package.json'])
...
gulp.task('clean', () => {
  return require('del')([PATHS.APP, PATHS.DIST])
})
...
gulp.task('package', gulp.series('build', async () => {
  try {
...
gulp.task('p', async () => {
  ...
  try {
    let { platform, arch } = process
    let ret = await executable(platform, arch, electronVersion)
    console.log(`finished building:`, ret)
  } catch (err) {
    onError(err)
    throw err
  }
