...
export default class ReplConsoleEnvironmentWatcher extends React.Component {
  ...
  getProperties (names) {
    ...
    return _.map(names, (key) => {
      ...
      let keyClass = Object.prototype.propertyIsEnumerable.call(context, key)
        ? 'env-key'
        : 'env-key dull'
      return (
  ...
  getENV () {
    let context = ReplContext.getContext()
    let userDefinedNames = _.sortBy(_.difference(Object.getOwnPropertyNames(context), ReplContext.getAlphaNames()))
    return _.partition(userDefinedNames, (name) => typeof context[name] ===
      'function')
  }
  ...
  render () {
    ...
    let valueClass = `repl-console-environment-listing-title fa ${this.state.valueView
      ? 'fa-minus-square-o'
      : 'fa-plus-square-o'}`
    let funClass = `repl-console-environment-listing-title fa ${this.state.funView
      ? 'fa-minus-square-o'
      : 'fa-plus-square-o'}`
    return (
      <div className='repl-console-environment'>
        <div className='repl-console-environment-head'>
          <span className='repl-console-environment-title flex'
                title='Watcher for user defined global variables'>
            Global Environment
          </span>
        </div>
        <div className='repl-console-environment-body'>
          <div className='repl-console-environment-body-header'>
            <span
              className='repl-console-environment-listing-title flex'>Values</span>
          ...
          </div>
          {
            this.state.valueView
              ...
              : <div className='repl-console-environment-no-data'> No
                values</div>
              : null
          }
          <div className='repl-console-environment-body-header'>
            <span
              className='repl-console-environment-listing-title flex'>Functions</span>
