...
export default class ReplOutputChartViewer extends React.Component {
  ...
  init () {
    this.chartViewable = this.props.chartViewable ||
      ReplCommon.candidateForChart(this.props.chart)
  ...
  generateColumnData (source) {
    ...
    return _.map(source, (v, k) => ([k].concat(v).concat(range)).slice(0, size +
      1))
  }
    ...
    return (
      <span className='repl-output-data-chart-viewer'>
        ...
        <span className='chart-viewer-preferences'>
          ...
          <i className={barClazz} title="Bar Chart"
             onClick={this.onClickBarChart}></i>
          <i className={lineClazz} title="Line Chart"
             onClick={this.onClickLineChart}></i>
          <i className={pieClazz} title="Pie Chart"
             onClick={this.onClickPieChart}></i>
          <i className={areaClazz} title="Area Chart"
             onClick={this.onClickAreaChart}></i>
          <span className="checkbox-group">
            <input type="checkbox" name="flip"
                   title='flip category and data'
                   checked={this.state.flip} value=""
                   onClick={this.onToggleFlip}/>
            flip
          </span>
          <span className="checkbox-group">
            <input type="checkbox"
                   title='rotate category and data'
                   name="rotate" checked={this.state.rotate} value=""
          ...
          <span className="checkbox-group">
            <input type="checkbox"
                   title='spline curve'
                   name="spline" disabled={!this.isSplineChart()}
                   checked={this.state.spline} value=""
                   onClick={this.onToggleSpline}/>
            spline
          </span>
          <span className='placeholder'></span>
        </span>
      ...
      this.state.chartCollapse
        ? <span className='repl-output-chart-viewer-container'>
            <i className='fa fa-plus-square-o'
               onClick={this.onToggleChartCollapse}></i>
