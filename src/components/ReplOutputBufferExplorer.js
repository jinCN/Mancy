...
export default class ReplOutputBufferExplorer extends React.Component {
  constructor (props) {
    ...
    _.each([
      'getGridHeaderOffsets', 'getASCIIOffsets', 'getGridData',
      'getGridRowData',
      'getGridRowOffset', 'onPreviousPage', 'onNextPage', 'hasNextPage',
      'hasPreviousPage',
  ...
  getGridHeaderOffsets () {
    ...
    return _.map(labels, (label) => <span
      className='offset-pos'>{label}</span>)
  }
  
  getASCIIOffsets () {
    return _.map(new Array(16), (l) => <span
      className='offset-ascii-pos'> </span>)
  }
  ...
  getGridRowData (row) {
    ...
    let getClazz = (clazz, printable, idx) => {
      ...
      return `${clazz} ${!printable ? 'mask' : ''} ${row % 2 == 0
        ? 'even'
        : 'odd'} ${pos < gridLength ? 'in' : 'out'}`
    }
    ...
    return (
      <div className='data-buffer-grid-row' key={rowKey}>
        ...
        {
          _.map(dataSet, (l, idx) => <span
            className={getClazz('offset-pos', true, idx)}>{toHexString(l)}</span>)
        }
        {
          _.map(dataSet, (l, idx) => {
  ...
  render () {
    ...
    let caretLeftClazz = `fa fa-caret-left ${this.hasPreviousPage()
      ? ''
      : 'disabled'}`
    let caretRightClazz = `fa fa-caret-right ${this.hasNextPage()
      ? ''
      : 'disabled'}`
    ...
    return (
      <div className='repl-output-data-buffer-explorer'>
        ...
        <div className='data-buffer-grid-pagination'>
          ...
          <i className={caretLeftClazz} title='Previous Page'
             onClick={this.onPreviousPage}></i>
          {this.state.page}/{this.pages}
          <span className='textbox'>
            <input type="number" name="page-number" title="Goto Page"
                   value={this.state.page} min="1" max={this.pages}
                   onChange={this.gotoPage}/>
          </span>
          <i className={caretRightClazz} title='Next Page'
             onClick={this.onNextPage}></i>
          <span className='placeholder'></span>
        </div>
      </div>
    )
