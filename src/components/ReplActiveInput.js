...
export default class ReplActiveInput extends React.Component {
  ...
  componentDidMount () {
    ...
    this.editor = CodeMirror(this.element, {
      ...
      gutters: preferences.toggleLineNumberGutter ||
      preferences.toggleFoldGutter
    ...
    cli.replMode = ReplLanguages.getREPLProvider()[`REPL_MODE_${(global.Mancy.session.mode ||
      global.Mancy.session.mode).toUpperCase()}`]
    //bind write handle
    ...
    return [
      {
        ...
        click: e => this.onTriggerAction({ action: 'undo' })
      },
      {
        label: 'Redo',
        accelerator: 'Shift+CmdOrCtrl+Z',
        click: e => this.onTriggerAction({ action: 'redo' })
      },
      { type: 'separator' },
      ...
      { label: 'Format', accelerator: 'Shift+Ctrl+F', click: this.onFormat },
      {
        ...
        enabled: global.Mancy.preferences.toggleFoldGutter &&
          !!this.editor.getValue().length
      },
      {
        label: 'UnFold All',
        accelerator: 'Shift+Ctrl+Q',
        click: this.onUnFoldAll,
        enabled: global.Mancy.preferences.toggleFoldGutter &&
          !!this.editor.getValue().length
      }
    ]
  ...
  onStoreChange (cmd) {
    ...
    let {
      now, activeSuggestion, breakPrompt,
      format, stagedCommands, autoComplete
  ...
  addEntryAction (action) {
    ...
    let addReplEntry = (output, formatted = false) => {
      ...
      formattedOutput = formatted ? output : ReplOutput.some(output)
        .highlight().formattedOutput
      const entry = _.extend({}, action, {
        formattedOutput,
        ns: ReplLanguages.getNamespace(),
        id: this.props.tag
      })
  ...
  prompt (preserveCursor) {
    ...
    if (cli.bufferedCommand.indexOf(this.replFeed) != -1 &&
      ...
      let { formattedOutput, error } = this.force
        ? { formattedOutput: cli.$lastExpression.getValue() }
        : cli.$lastExpression.highlight(this.commandOutput)
      if (!this.replFeed) {
  ...
  addEntry (buf) {
    ...
    if (output.length === 0 || output.indexOf('at REPLServer.complete') !==
      -1) {
      return
    }
...
//    const text = cm.getValue()
    ...
    if (suggestions.length && completeEntry(suggestions, code)) {
      ...
      ReplSuggestionActions.addSuggestion({
  ...
  transpileAndExecute (err, result, transform = (e, x) => x) {
    ...
    if (err) {
      ...
      const transpileCallback = (err, output, time) => {
        if (err && this.canRetry(err)) {
          this.execute(true)
        } else {
          let transformedOutput = transform(err, output)
          let { formattedOutput } = this.force && !err
            ? { 'formattedOutput': transformedOutput }
            : ReplOutput.some(err || transformedOutput).highlight()
          let transpiledOutput = !this.shouldTranspile()
            ? null
            : ReplOutput.transpile(result)
          this.addEntryAction({
            formattedOutput,
            status: !err,
            command: ReplCommon.highlight(text),
  ...
  traverseHistory (direction = -1) {
    ...
    let navigateHistory = (up, cmd, pos) => {
      ...
      this.reloadPrompt(cmd, cursor, pos, (pos === -1
        ? ''
        : this.history.staged))
    };
  ...
  complete (callback, allowEmpty = false) {
    ...
    if (!allowEmpty && !beforeCursor.trim().length) {
      return
    }
  ...
  render () {
    return (
      <div className='repl-active-input' id={this.props.tag}
           key={this.props.tag}>
      </div>
    )
  }
