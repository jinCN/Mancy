...
describe('ReplEntryOutputError', () => {
  ...
  it('should rendered properly', () => {
    ...
    expect(React.findDOMNode(errorTraceFunction).textContent)
      .toContain(errorFunction)
    expect(React.findDOMNode(errorTraceLine).textContent).toContain(errorLine)
    expect(React.findDOMNode(errorTraceColumn).textContent)
      .toContain(errorColumn)
  })
})
