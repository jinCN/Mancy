...
export default class ReplOutputGridViewer extends React.Component {
  constructor (props) {
    ...
    if (!this.gridViewable) {
      try {
        this.gridViewable = ReplCommon.candidateForGrid(this.props.grid)
      } catch (e) {
      }
    }
    
    _.each([
      'onToggleCollapse', 'onToggleTranspose', 'renderGrid',
      'transposeGridData',
  ...
  gridData () {
    ...
    return this.gridValues ||
      (this.gridValues = _.map(_.keys(grid), (key) => _.values(grid[key])))
  }
  ...
  renderGrid () {
    ...
    let data = this.state.transpose
      ? this.transposeGridData()
      : this.gridData()
    const clazzMap = {
      number: 'cm-number',
      boolean: 'cm-atom',
              ...
              return (
                <tr>
                  {<th>{head}</th>}
                  {
                    _.map(data[pos], (cell) => {
                      return <td
                        className={clazzMap[typeof cell]}>{this.getCellString(cell)}</td>
                    })
                  }
                </tr>
              )
