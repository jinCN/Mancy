...
export default class ReplEntryMessage extends React.Component {
  ...
  showExecutionTime () {
    ...
    let clazz = `fa fa-clock-o execution-time ${s > 10 ? 'red' : (s > 5
      ? 'orange'
      : 'green')}`
  ...
  render () {
    ...
    if (this.props.commandCollapse) {
      ...
      if (lines.length > 1 || lines[0].length >
        ReplConstants.COMMAND_TRUNCATE_LENGTH) {
    ...
    return (
      <div className='repl-entry-message'>
        <div className='repl-entry-command-container'>
          ...
          {this.props.message.ns
            ? <span className='tag'
                    title='Namespace'>{this.props.message.ns}</span>
            : null}
