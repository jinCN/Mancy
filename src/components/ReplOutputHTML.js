...
export default class ReplOutputHTML extends React.Component {
  ...
  onLoadIFrame () {
    ...
    let height = parseInt(styles.height) + parseInt(styles.marginTop) +
      parseInt(styles.marginBottom)
    // fix max height
    iframe.height = Math.min(height, ReplConstants.IFRAME_MAX_HEIGHT) + 'px'
    doc.body.style.color = (document.body.className.indexOf('dark-theme') !== -1
      ? 'whitesmoke'
      : 'darkslategrey')
  }
  ...
  render () {
    ...
    return (
      <span className='repl-html-fold'>
        ...
        {
          this.state.html
            ? <iframe className='sandbox-view' id={this.id}
