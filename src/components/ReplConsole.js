...
export default class ReplConsole extends React.Component {
  ...
  render () {
    ...
    return (
      <div className='repl-console-message'>
            ...
            _.map(this.state.entries.filter(v=>this.state[v.type]), ({ type, data, time, count }) => {
              return (
                <div
                  ...
                  key={time}>
                  {count > 1
                    ? <span className='console-message-count'
                            title={this.getDupTooltip(count, type)}
                            style={this.getDupCountStyle(count)}>{count}</span>
                    : null}
                  {ReplConsole.getTypeIcon[type]}
                  <span
                    className={this.getTypedClassName('repl-console-message-entry-content', type)}>
                  {data}
                </span>
                </div>
              )
        ...
        <div className="repl-console-environment-resizeable"
  ...
  static getTypeIcon = (() => {
    let cache = {}
    cache.error =
      <i className="fa fa-exclamation-circle repl-console-message-error"
         title='error'></i>
    cache.warn =
      <i className="fa fa-exclamation-triangle repl-console-message-warning"
         title='warning'></i>
    cache.info = <i className="fa fa-info-circle repl-console-message-info"
                    title='info'></i>
    return cache
  })()
