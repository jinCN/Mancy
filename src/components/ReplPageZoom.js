...
const zoomOptions = [
  0.75, 0.9, 1, 1.1, 1.2, 1.25, 1.3, 1.4, 1.5, 1.6, 1.7, 1.75, 1.8, 1.9, 2, 2.1,
    ...
    return (
      <div className='preference'>
        ...
        <div className='preference-value'>
          <select onChange={this.onChangePageZoomFactor}
                  title='Page Zoom Factor'>
            {
              _.map(zoomOptions, (z) => {
                return <option selected={z === zoom}
                               value={z}>{this.getZoomPercentage(z)}</option>
              })
            }
          </select>
        </div>
